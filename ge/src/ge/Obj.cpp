...
unsigned int updateVertex(
	...
	const std::vector<glm::vec2>& in_texcoords,
	const vertex_index& i)
{
	...
	assert(in_positions.size() > (unsigned int)(i.v_idx));
	...
	unsigned int idx = positions.size() - 1;
	vertexCache[i] = idx;
	...
	const bool is_material_seted)
{
	...
	//
	shape.name = name;
	shape.mesh = std::shared_ptr<ge::Mesh>(new Mesh);
	shape.mesh->vertices.swap(positions);
	shape.mesh->normals.swap(normals);
	shape.mesh->uv.swap(texcoords);
	shape.mesh->triangles.swap(indices);
  ...
  std::istream& inStream)
{
  ...
  while (inStream.peek() != -1) {
    ...
    if ((0 == strncmp(token, "newmtl", 6)) && isSpace((token[6]))) {
      ...
      material_map.insert(std::pair<std::string, ObjMaterial>(material.name, material));
