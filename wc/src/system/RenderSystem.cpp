...
RenderSystem::RenderSystem(ge::Renderer* renderer, int width, int height) : EntityProcessingSystem(artemis::Aspect::getAspectFor<RenderComponent>()), _renderer(renderer), _changeWireframe(false), _wireframe(false), _showBounds(false), _showScene(true), _time(0.0f) {
	...
	_mainCamera.setTransform(_mainCamera.getTransform().setTranslation(glm::vec3(0.0f, 200.0f, 50.0f)).lookAt(glm::vec3(0.0f, 0.0f, 0.0f)));
...
void RenderSystem::initialize() {
	shared_ptr<resource::Resource> resource = resource::loadobj("axis.nff", "axis");
	_axis = resource->getMeshR();
...
void RenderSystem::processEntity(const std::shared_ptr<artemis::Entity>& e) {
	...
	for (size_t i = 0; i < renderComponent->renderable.size(); ++i) {
		...
		for (size_t i = 0; i < renderable->getNumSubmeshes(); ++i) {
			renderable->getMaterial(i).setColor("l_direction", _lightDir);
			renderable->getMaterial(i).setColor("l_diffuse", lightDiffuse);
			renderable->getMaterial(i).setColor("l_ambient", lightAmbient);
		}
...
void RenderSystem::begin() {
	...
	_lightDir = _mainCamera.getModelViewMatrix() * glm::vec4(0.0f, 0.8f, 0.0f, 0.0f);
...
void RenderSystem::inserted(const shared_ptr<artemis::Entity>& e) {
	RenderComponent* renderComponent = renderMapper.get(e);
	if (renderComponent) {
