...
int MicroPather::SolveForNearStates( void* startState, MP_VECTOR< StateCost >* nnear, float maxCost )
{
...
#ifdef DEBUG
	for( unsigned i=0; i<nnear->size(); ++i ) {
		for( unsigned k=i+1; k<nnear->size(); ++k ) {
			MPASSERT( (*nnear)[i].state != (*nnear)[k].state );
		}
	}
#endif
